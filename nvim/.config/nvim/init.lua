...
-- Treat Bazel BUILD/WORKSPACE/.bzl files as starlark (filetype `bzl`)
vim.filetype.add {
  extension = {
    bzl = 'bzl',
    bazel = 'bzl',
...
-- Stop all gopls clients and re-fire FileType on every loaded Go buffer so a fresh client attaches.
...
vim.keymap.set('n', '<leader>bg', function()
  ...
  vim.system({ 'bazel', 'run', '//:gazelle' }, { text = true }, function(out)
    vim.schedule(function()
      if out.code == 0 then
        restart_gopls()
        vim.notify('gazelle done; gopls restarted', vim.log.levels.INFO)
      else
        vim.notify('gazelle failed:\n' .. (out.stderr or ''), vim.log.levels.ERROR)
      end
    end)
  end)
end, { desc = '[B]azel: run [G]azelle + restart gopls' })
...
require('lazy').setup({
  ...
  -- LSP Plugins
  {
    ...
    config = function()
      ...
      -- Resolve bazel workspace once (from nvim's cwd at startup).
